...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON games.winner_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM teams INNER JOIN games ON games.opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
